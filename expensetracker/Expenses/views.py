...
def post_expense(request):
    data = {key: value[0] for key, value in dict(request.data).items() if 'image' not in key}
    tags = data.pop('tags').split(',')
    images = request.FILES
    catid = Category.objects.filter(name=data['category']).first().id
    data = {**data, 'category': catid, 'userid': decode_uuid(data['userid']), 'amount': round(float(data['amount']), 2)}
    ser_data = PostExpenseSerializer(data=data)
    ser_data.is_valid(raise_exception=True)
    ser_data.save()
    return ser_data.data, 201
