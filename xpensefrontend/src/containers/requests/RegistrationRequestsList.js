...
const fieldStyleMapper = {
    ...
    role: "w-24", created_at: "w-44", employee_id: "w-44", authorized: "w-24", comment: "w-96 h-10 overflow-scroll py-2"
  }
...
<>
  ...
  return (
    <>
      ...
      {!requests.length && <div className="w-full h-full flex-col-style justify-start bg-gray-100 text-xl font-light"><span>No users found</span></div>}
      {requests.length && 
      <>
        <table className={table}>
          <TableCaption caption={`User Details (${requests.length})`} />
          ...
          <tbody className={`${tbody}`}>
            {requests.map((obj, idx) => <View key={idx} obj={obj} keys={keys} fieldStyleMapper={fieldStyleMapper}/>)}
          </tbody>
        </table>
...
function View({obj, keys, fieldStyleMapper}){
  ...
  const requests = useSelector(state => state.expense.requests);
...
//                     <table className="">
//                         <caption className="caption-top py-2 text-xl text-gray-500 font-semibold ">Registration Requests</caption>
//                         <thead className={`${thead}`}>
//                             <tr>
//                             <th><i className="fa fa-circle opacity-0"></i></th>
//                             </tr>
