...
// Styles
const outerdiv = "sticky top-0 left-0 right-0 border-b-2 pb-2 bg-white";
...
const innerdiv_button = "text-sm border rounded-md flex-row-style justify-center p-2 cursor-pointer hover:opacity-80";
    ...
    <tr className={`${tr}`}>
      {keys.map((ele, idx) => (
        <td key={idx} className={`${td} ${fieldStyleMapper[ele]}`}>
          {(ele.includes("date") || ele.includes("last_modified"))
            ? dateformater(obj[ele])
            : obj[ele]}
        </td>
      ))}
      {obj.status === "pending" && (
    ...
    <>
        ...
        {toggleFilterHeader && (
          <>
            ...
            <div className="w-full h-fit flex-row-style justify-between mt-2 px-2 bg-white">
              <span className="flex-row-style justify-end flex-grow px-4">
                <Link to={"./submit/addexpense"} className={innerdiv_1_2_add}>
                  + Add expense
                </Link>
              </span>
      ...
      <table className={table}>
        ...
        <tbody className={`${tbody}`}>
          {expenselist.map((obj, idx) => (
            <View key={idx} obj={obj} keys={keys} fieldStyleMapper={fieldStyleMapper} setviewobj={setViewexpenseobj} />
          ))}
        </tbody>
      </table>
