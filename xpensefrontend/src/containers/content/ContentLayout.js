...
const innerdiv_1 = (showTags, showSummary) => `border border-t-0 border-b-0 ${showTags && showSummary ? 'w-[85%]' : 'w-full'} h-full rounded-lg shadow shadow-lg flex-col-styleoverflow-x-scroll overflow-y-scroll`;
...
function BaseDisplay() {
    ...
    return (
        <div className={outerdiv}>
            {/* Left */}
            <div className={innerdiv_1(showTags, showSummary)}>
                <div className={inner_1_1}> {typeContentMapper[type]} </div>
            </div>
            {/* right */}
            <div className={innerdiv_2(showTags, showSummary)}>
                {/* right top */}
