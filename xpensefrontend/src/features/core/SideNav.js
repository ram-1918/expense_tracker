    ...
    return (
        <div className={`w-full flex-col-style justify-start space-y-8`}>
            ...
            <div className='w-full flex-col-style justify-start'>
                ...
                {showSideNav ? <span onClick={() => setActive('myexpenses')} className={`${commons('myexpenses')}`}><Link to={`/user/${userid}/home/manage/expenses`}><FontAwesomeIcon icon={faWallet} /> My Expenses</Link></span> : <span onClick={() => setActive('myexpenses')}  className={commons('myexpenses')}><Link to={`/user/${userid}/home/manage/expenses`}><FontAwesomeIcon icon={faWallet} /></Link></span>}
                {showSideNav ? <span onClick={() => setActive('receipts')} className={`${commons('receipts')}`}><FontAwesomeIcon icon={faReceipt} /> Reciepts</span> : <span className={commons('receipts')}><FontAwesomeIcon icon={faReceipt} /></span>}
                {showSideNav ? <span onClick={() => setActive('reports')} className={`${commons('reports')}`}><FontAwesomeIcon icon={faChartBar} /> Reports</span> : <span className={commons('reports')}><FontAwesomeIcon icon={faChartBar} /></span>}
                {showSideNav ? <span onClick={() => setActive('forecasts')} className={`${commons('forecasts')}`}><FontAwesomeIcon icon={faChartLine} /> Forecasts</span> : <span className={commons('forecasts')}><FontAwesomeIcon icon={faChartLine} /></span>}
            </div>
            <div className='w-full flex-col-style justify-start'>
