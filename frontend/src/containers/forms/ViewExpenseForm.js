...
import { changeexpensestatus, fetchsingleuser, fetchusers, updateuserinfo, allexpenseslist } from '../../features/core/state/coreThunks';
...
function ViewExpenseForm() {
    ...
    const [showMessageBlock, setShowMessageBlock] = useState(null);
    ...
    useEffect(() => {
        ...
        if(type === 'expenses'){
            if(location.pathname.includes('requests')){
                // expense requests
                singleexpense = expenserequests.filter((obj) => obj.id === expenseid)[0];
            } else {
                // expense requests
            ...
            // allexpenses
            singleexpense = allexpenses.filter((obj) => obj.id === expenseid)[0];
        }
        if (singleexpense) {
    ...
    const handleProofUpdate = async () => {
        ...
        try{
            ...
            console.log("error in handle proof update");
            setSpinner(false);
    ...
    const handleTags = async () => {
        ...
        try{
            ...
            console.log("error in handle tags update");
            setSpinner(false);
    ...
    const statusStyleMapper = {
        'pending': 'capitalize text-blue-600 font-medium',
    ...
    const cantDeleteButton = <span onClick={() => {handleDelete(expense.id)}}><i className='fa fa-trash text-green-500'></i></span>
    const statusDiv = <span className={statusStyleMapper[expense['status']]}>{expense['status']}</span>;

    const isAdmin = () => (userinfo['role'] === 'superadmin') || (userinfo['role'] === 'admin');
    const sameUser = () => expense['userid'] === userid;
    const notSameUser = () => expense['userid'] !== userid

    function handleStatusDisplay() {
        if(sameUser()){
            if (expense['status'] === 'pending') return <>{!updateMode ? <>{beforeUpdateOptions} </> : <>{afterUpdateOptions}</>} {statusDiv} {expense['rejection_count'] < 1 ? deleteButton : cantDeleteButton}</>
                ...
                if (expense['status'] === 'pending'){
                    return (
                        <>
                            ...
                            <span onClick={() => setShowMessageBlock(true)} className='btn-delete text-sm'>Reject</span>
    ...
    return (
        <div className={`z-30 fixed top-0 left-0 bottom-0 w-[100%] h-full flex-col-style justify-start bg-[rgba(0,0,0,0.8)] text-sm`}>
            ...
            <div className='w-fit h-fit shadow-lg rounded-lg flex-col-style justify-center m-2 bg-white'>
                <div className='border-b rounded-tr-lg rounded-tl-lg w-full flex-row-style justify-between px-2 py-1 bg-slate-100'>
                ...
                <div className='border w-full flex flex-row-style justify-evenly flex-grow px-2 '>
                    <div className='border-r w-fit h-fit px-2 '>
                        ...
                        <div className='flex-row-style justify-around'>
                            ...
                            {updateMode && 
                            <>
                                <span onClick={() => {handleProofUpdate()}} className='cursor-pointer border rounded-md p-2 bg-slate-200 my-8 mx-2 hover:opacity-80'>Replace Bills</span>
                            </> 
                            }
                        </div>
                    </div>
                        ...
                        <table className='w-full h-full '>
                            <tbody>
                                ...
                                <tr key={idx} className='w-full px-2'>
                                    <td className='text-base font-medium text-left'>{expense[key] && keyMapper[key]}</td>
                                    <td className={`${key.includes('amount') ? 'before:content-["$"] before:font-bold' : ''} text-base font-normal px-2 text-left`}>
                                        {(key.includes('date_submitted') || key.includes('last_modified')) ? dateformater(expense[key]) : 
                                        (key.includes('username') && expense[key]) ? capitalize(expense[key]) :
                                        (key.includes('message') && expense[key]) ? expense[key] :
                                        expense[key]}
                                    </td>
                                </tr>
                                ))}
                ...
                <div className='border-t border-b w-full flex-row-style justify-between p-2'>
                ...
                {showMessageBlock && <div className='w-full flex-row-style justify-center space-x-2 p-4'>
                    <label>Add a note? </label>
                    <textarea rows="2" placeholder='Add a note within 200 charecters...' className='w-96 min-h-full border border-gray-300 outline-none px-2' value={expenseMessage} onChange={(e) => setExpenseMessage(e.target.value)} />
                    <button type='button' className="btn-delete text-sm" onClick={() => handleRequest({status:'reject', 'message': expenseMessage})} >Proceed</button>
                </div>}
            </div>
        </div>
    )
