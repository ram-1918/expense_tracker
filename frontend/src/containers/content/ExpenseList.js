...
import { setActiveSideNav } from "../../features/core/coreSlice";
...
const td = 'border-r overflow-x-hidden flex-row-style text-sm text-left text-gray-700 px-2 py-[5px]';
...
function ExpensesList({activecolumns}) {
  ...
  console.log('manage/expenses', expenselist || []);
  ...
  const titleMapper = {
    ...
    rejection_count: "Rejection Times",
  ...
  useEffect(() => {
    dispatch(setActiveSideNav('myexpenses'));
  ...
  return (
    <div className="w-full">
      {!expenselist.length && <div className="justify-start w-full h-full text-xl font-light bg-gray-100 flex-col-style"><span>No expenses yet</span></div>}
      ...
      <table className={table}>
        ...
        <tbody className={`${tbody}`}>
          {/* {!activecolumns.length && <div className="justify-start w-full h-full text-xl font-light bg-gray-100 flex-col-style">No Column Selected</div>}  */}
          {/* {!expenselist.length && <div className="justify-start w-full h-full text-xl font-light bg-gray-100 flex-col-style"><span>No expenses yet</span></div>} */}
          {activecolumns.length && expenselist.map((obj, idx) => (
            <View key={idx} obj={obj} keys={activecolumns} fieldStyleMapper={fieldStyleMapper} />
          ))}
        </tbody>
      </table>
      </>}
...
function View({ obj, keys, fieldStyleMapper}) {
  ...
  return (
    <tr className={`${tr}`}>
      {keys.map((ele, idx) => (
        <td key={idx} className={`${td} ${fieldStyleMapper[ele]} ${statusColorMapper[obj[ele]]}`}>
       ...
       {(obj.status === 'pending' && obj.rejection_count < 1) ? 
          ...
          : 
          <td className={`${td1} relative z-0 p-2 flex-row-style justify-center after:content-[''] after:absolute after:px-0 after:py-3 after:border after:border-slate-400 after:rotate-45 `}>
            <i className="fa fa-trash"></i>
          </td> 
        }
      {
...
//         <div className="justify-between px-4 flex-row-style">
...
//           <div className="justify-between w-full px-2 mt-2 bg-white h-fit flex-row-style">
//             <span className="justify-end flex-grow px-4 flex-row-style"><Link to={'./submit/addexpense'} className={innerdiv_1_2_add}>+ Add expense</Link></span>
