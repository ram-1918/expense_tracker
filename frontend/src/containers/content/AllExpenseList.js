...
const table = "table-auto rounded-md m-auto";
...
const tr = (myexpense) => `border w-full h-8 flex-row-style justify-between py-5 cursor:pointer odd:bg-white even:bg-slate-100 hover:border-teal-500 ${myexpense && 'border-blue-400'}`;
...
function AllExpensesList({activecolumns}) {
  ...
  useEffect(() => {
    dispatch(setActiveSideNav('allexpenses'));
  ...
  return (
    <div className="w-full">
      {spinner && <Spinner name="Loading expenses..." />}
...
function View({ obj, keys, fieldStyleMapper }) {
  ...
  const is_my_expense = obj['userid'] === userid;
  return (
    <tr className={`${tr(is_my_expense)}`}>
      {keys.map((ele, idx) => (
