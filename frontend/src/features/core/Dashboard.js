...
import { fetchsummaries } from './state/coreThunks';
...
function Dashboard() {
    ...
    useEffect(() => {
        dispatch(setActiveSideNav('dashboard'));
        dispatch(fetchsummaries());
    }, [])

    return (
          <div className="border-0 w-[97%] h-full flex-col-style justify-around space-y-4 pb-8 overflow-y-scroll">
...
function DisplayApprovals(){
    ...
    const receipt = [
        {"name": "Expense1", "date": "September 7th, 2023", "by":"Person1", "status": "3", "amount": "213.00", "count": 2},
        {"name": "Expense2", "date": "August 30, 2023", "by":"Person2", "status": "3", "amount": "43.23", "count": 0}
    ]
    return (
